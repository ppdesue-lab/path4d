...
public:
  ...
  bool containXY(const glm::vec3& point) const;
  ...
  void multiply(const glm::vec3& dir);
  ...
  void setMax(const glm::vec3& max_pt) { mMax = max_pt; }
  ...
  /// <param name="bb"></param>
  /// <returns></returns>
  AABB overlappedBox(const AABB& bb) const;
  ...
  std::vector<glm::vec3> getCorners(const glm::vec3& offset=glm::vec3(0,0,0)) const;
private:
